...
const defaultConfigSyncDirectories = ["plugins"];
const modConfigSyncDirectories = ["config", "plugins"];

function configSyncDirectoriesFor(platform) {
  return ["fabric", "forge"].includes(platform)
    ? [...modConfigSyncDirectories]
    : [...defaultConfigSyncDirectories];
}
...
const form = reactive({
  ...
  configSyncDirectories: [...defaultConfigSyncDirectories],
...
function resetForm() {
  ...
  Object.assign(form, {
    ...
    configSyncDirectories: source?.config_sync_directories?.length
      ? [...source.config_sync_directories]
      : configSyncDirectoriesFor(source?.platform || "paper"),
...
watch(() => form.kind, (value) => {
  ...
  if (value !== "proxy" && !["paper", "spigot", "fabric", "forge"].includes(form.platform)) form.platform = "paper";
  if (open.value && !editing.value && !resettingForm) void loadProxyTargetDefaults();
});
watch(() => form.platform, (value) => {
  if (open.value && !editing.value && form.kind === "mirror" && !resettingForm) {
    form.configSyncDirectories = configSyncDirectoriesFor(value);
  }
});
watch(() => form.nodeId, () => {
...
async function submit() {
  ...
  if (form.kind === "mirror") {
    ...
    if (!form.pluginConfigSyncExtensions.length) {
      ElMessage.warning("请至少保留一个配置文件后缀");
      return;
    }
    if (!form.configSyncDirectories.length) {
      ElMessage.warning("请至少保留一个配置同步目录");
      return;
    }
  }
  ...
  if (form.kind !== "mirror") {
    ...
    Object.assign(config, {
      ...
      config_sync_directories: form.configSyncDirectories,
    });
  }
  emit("submit", {
    nodeId: form.nodeId,
  ...
  >
    <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
      ...
      <template v-else>
        ...
        <el-form-item label="配置同步目录">
          <el-select
            v-model="form.configSyncDirectories"
            class="full-control"
            multiple
            filterable
            allow-create
            ...
            placeholder="输入目录名并回车，例如 config"
          >
            <el-option v-for="directory in ['plugins', 'config']" :key="directory" :label="directory" :value="directory" />
          </el-select>
          <small class="form-help">同步配置时从镜像源覆盖这些根目录到各实例；mod 服（Fabric/Forge）默认同步 config 与 plugins，Bukkit 系默认仅 plugins。</small>
        </el-form-item>
        <el-form-item label="配置文件后缀白名单">
          <el-select
          ...
          <small class="form-help">仅同步这些后缀的配置文件，不会删除目标中的额外文件；数据库后缀请勿加入白名单。</small>
        </el-form-item>
      </template>

...
import { ArchiveRestore, Package, Plus, RefreshCw, Search, Trash2, Upload } from "lucide-vue-next";
...
const uploadForm = ref({ pluginType: "spigot", autoInstall: false });
...
function resetUpload() {
  ...
  uploadForm.value = { pluginType: "spigot", autoInstall: false };
}
...
async function removePlugin(plugin) {
  try {
    await ElMessageBox.confirm(
      `将删除“${plugin.name}”及其全部仓库制品，已部署到服务器的插件不会受影响。此操作不可恢复。`,
      "删除插件",

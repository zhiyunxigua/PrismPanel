...
func (s *Server) handleServer(writer http.ResponseWriter, request *http.Request) {
	...
	if len(parts) == 3 && parts[1] == "plugin-config-sync" && parts[2] == "detect" {
		s.handleConfigSyncDetect(writer, request, parts[0])
		return
	}
...
func (s *Server) handlePluginConfigSync(writer http.ResponseWriter, request *http.Request, serverID string) {
	...
	if err == nil {
		err = s.callNode(request, "deployment.plugin_config_sync.start", map[string]any{
			"server_id": serverID, "targets": input.Targets, "directories": input.Directories,
		}, &result)
	}
...
// handleConfigSyncDetect 自适应检测镜像源中可同步的配置目录（plugins/config），
// 供前端按插件服/mod 服自动选择同步根；无法确定时返回 issues 供前端弹窗说明。
func (s *Server) handleConfigSyncDetect(writer http.ResponseWriter, request *http.Request, serverID string) {
	...
	var result json.RawMessage
	err := s.callNode(request, "config_sync.detect", map[string]any{"server_id": serverID}, &result)
	if err != nil {
		writeError(writer, err)
		return
	}

...
type task struct {
	...
	force   bool
	release func()
	// directories 为配置同步任务显式指定的同步根目录（空则用服务器配置的默认值）。
	directories []string
}
...
func (m *Manager) Start(serverID string, requested []int) (Snapshot, error) {
	return m.start(serverID, requested, TaskKindMirrorDeploy, nil)
}

// StartPluginConfigSync 启动配置同步任务。directories 可选：显式指定要同步的
// 镜像源根目录（由前端自适应检测后传入）；为空时使用服务器配置的默认同步根。
// 显式目录必须通过校验（拒绝绝对路径/.. 逃逸，且必须属于该服务器允许的配置目录集合），
// 防止恶意调用把同步范围扩大到镜像源之外的路径。
func (m *Manager) StartPluginConfigSync(serverID string, requested []int, directories []string) (Snapshot, error) {
	if len(directories) > 0 {
		cfg, err := m.servers.Get(serverID)
		if err != nil {
			return Snapshot{}, err
		}
		if err := validateSyncDirectories(cfg, directories); err != nil {
			return Snapshot{}, err
		}
	}
	return m.start(serverID, requested, TaskKindPluginConfigSync, directories)
}

// validateSyncDirectories 校验配置同步的显式目录：
//   - 必须是相对路径且不含 .. 逃逸（filepath.Clean 后仍位于镜像源根下）；
//   - 必须属于该服务器允许的配置目录集合（配置的 config_sync_directories，
//     未配置时按平台默认：mod 服 ["config","plugins"]，插件服 ["plugins"]）。
func validateSyncDirectories(cfg model.ServerConfig, directories []string) error {
	allowed := make(map[string]struct{})
	for _, dir := range cfg.ConfigSyncDirectories {
		allowed[filepath.Clean(dir)] = struct{}{}
	}
	if len(allowed) == 0 {
		if model.IsModPlatform(cfg.Platform) {
			for _, dir := range model.ModConfigSyncDirectories {
				allowed[dir] = struct{}{}
			}
		} else {
			for _, dir := range model.DefaultConfigSyncDirectories {
				allowed[dir] = struct{}{}
			}
		}
	}
	for _, raw := range directories {
		...
		if dir == "" || dir == "." || filepath.IsAbs(dir) ||
			dir == ".." || strings.HasPrefix(dir, ".."+string(filepath.Separator)) {
			return apperr.New("INVALID_REQUEST", fmt.Sprintf("非法同步目录：%s", raw))
		}
		if _, ok := allowed[dir]; !ok {
			return apperr.New("INVALID_REQUEST", fmt.Sprintf("同步目录 %s 不在该服务器允许的配置目录列表中", raw))
		}
	}
	return nil
}

func (m *Manager) start(serverID string, requested []int, kind string, directories []string) (Snapshot, error) {
	cfg, err := m.servers.Get(serverID)
	if err != nil {
		return Snapshot{}, err
	}
	...
	item := &task{
		...
		context: ctx, cancel: cancel, directories: directories, release: func() {
			release()
			imageLock.RUnlock()
		},
	}
	...
	for _, slot := range item.Targets {
		...
		if item.Kind == TaskKindPluginConfigSync {
			...
			if snapshotErr != nil {
				...
				err = m.syncConfigFiles(item, cfg, instanceID, snapshot.Workspace, item.directories)
			}
		} else {
